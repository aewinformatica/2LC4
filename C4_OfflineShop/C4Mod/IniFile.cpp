...
bool CIniFile::Save(string FileName, vector<Record>& content)
{
	...
	for (s32 i=0;i<(s32)content.size();i++)									// Loop through each vector
	{
...
string CIniFile::Content(string FileName)
{
	...
	{
		for (s32 i=0;i<(s32)content.size();i++)								// Loop through the content
		{
...
s32 CIniFile::GetIntValue(string KeyName, string SectionName, string FileName)
{
	return atoi(CIniFile::GetValue(KeyName, SectionName, FileName).c_str());
}

s32 CIniFile::GetBoolValue(string KeyName, string SectionName, string FileName)
{
...
bool CIniFile::DeleteSection(string SectionName, string FileName)
{
	...
	if (Load(FileName, content))											// Make sure the file is loaded
	{
		for(s32 i=(s32)content.size()-1;i>-1;i--)								// Iterate backwards through the content
		{							
